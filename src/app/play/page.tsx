...
function PlayPageClient() {
  ...
  useEffect(() => {
    ...
    if (currentSubtitles.length > 0) {
      // 使用 getPreferredSubtitle 优先选择中文字幕
      const preferredSubtitle = getPreferredSubtitle(currentSubtitles) as SourceSubtitleItem;
      currentSubtitleLabelRef.current = preferredSubtitle?.label || '关闭';
      void switchSourceSubtitle(preferredSubtitle).catch((error) => {
  ...
  useEffect(() => {
    ...
    const initPlayer = async () => {
      try {
        ...
        // 使用 getPreferredSubtitle 优先选择中文字幕
        const preferredSubtitle = getPreferredSubtitle(currentSubtitles) as SourceSubtitleItem;
        const shouldUseNativeInitialSubtitle = !!preferredSubtitle && !isAdvancedSourceSubtitle(preferredSubtitle);
        ...
        artPlayerRef.current.on('ready', async () => {
          ...
          const readyPreferredSubtitle = getPreferredSubtitle(readySubtitles) as SourceSubtitleItem;
          if (readyPreferredSubtitle && isAdvancedSourceSubtitle(readyPreferredSubtitle)) {
            void switchSourceSubtitle(readyPreferredSubtitle)
              .catch((error) => {
